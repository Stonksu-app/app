...
- **De `auth` y `storage` se copian datos, nunca estructura.** Esas tablas las provisiona Supabase y su servicio espera su propia versión. Quedan fuera `auth.schema_migrations`, que dice en qué versión está el servicio, y `auth.instances`, que identifica la instancia de GoTrue del proyecto: machacarla con la de dev deja la autenticación de test hablando de un proyecto que no es el suyo. Si los dos proyectos van con versiones distintas del servicio de auth, el clon falla por una columna que no existe: actualiza el atrasado desde el panel.
