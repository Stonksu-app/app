...
export default function Home() {
  ...
  const { name, isNodeUnlocked, isLessonCompleted, hasSeenIntro } = useUserStore();
  ...
  const nodesWithStatus = useMemo(
    () =>
      SKILL_TREE.map((node) => {
        ...
        return { node, unlocked, hasLessons, completed };
      }),
    [isNodeUnlocked, isLessonCompleted]
  );
    ...
    <div className="min-h-screen bg-carbon-900">
      ...
      <div className="max-w-2xl mx-auto px-4 pb-20">
        <div className="relative" style={{ height: totalHeight }}>
          ...
          {nodesWithStatus.map(({ node, unlocked, completed }) => {
          ...
          >
            ...
            {selected.lessons.length > 0 ? (
              <button
                ...
                className="mt-6 w-full bg-lime-500 hover:bg-lime-400 text-carbon-900 font-black py-3.5 rounded-2xl transition active:scale-95 flex items-center justify-center gap-2"
              >
