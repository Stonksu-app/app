...
| Backend | `supabase/migrations/0001…0016` | Esquema, RLS, RPC, triggers y cron |
...
Los campos de progreso nuevos requieren coordinar store, `snapshot()`, `CloudState`, `toRow/fromRow` y SQL. El rango/grupo/semana de liga vienen del servidor; el cliente sube XP semanal. El simulador usa una sincronización transaccional independiente (ver abajo); el límite de práctica de vocabulario y los ajustes del dispositivo siguen siendo locales. La lógica contempla conflictos de apodo, cuentas eliminadas y un reintento sin una columna ausente.
...
El menú de operar/escala se abre con clic derecho en escritorio y, en móvil (sin clic derecho), manteniendo pulsado ~450 ms sin desplazar el dedo más de ~10 px (`PriceChart.tsx`, `LONG_PRESS_MS`/`LONG_PRESS_SLOP_PX`); superado ese margen se interpreta como el inicio de un arrastre del gráfico y se cancela. Es puntero táctil/lápiz, no ratón, para no interferir con el arrastre normal del botón izquierdo. El engranaje de la escala sigue siendo la única vía táctil para ajustes de escala sin precio (abre el menú sin posición).
...
Long/short, apalancamiento 1–100x, margen aislado/cruzado, órdenes market/limit, take profit/stop loss, comisiones maker/taker y liquidación. Al volver se consulta el recorrido de velas para resolver ejecuciones/salidas. Es una simulación con aproximaciones: el cliente sigue evaluando el mercado, no hay un motor de precios ejecutándose en el servidor.

`useSimulatorSync.ts` y `lib/simulatorSync.ts` guardan posición, orden pendiente, últimas 50 operaciones y contador/fecha de operaciones diarias en `profiles.simulator_state`. La migración `0016_simulator_sync.sql` añade `simulator_read` y `simulator_commit`: bloquean la fila y comparan `simulator_revision` para que una apertura/cierre simultáneo solo se acepte una vez. El cierre y su efecto en monedas se confirman juntos. El guardado general no escribe el documento; incluye la revisión, y un trigger impide que un guardado atrasado deshaga el saldo de un cierre. Antes de operar se guarda el saldo local pendiente de sincronización.

La pantalla deriva la posición del store (ya no de un estado privado inicializado solo al montar); consulta al entrar, al recuperar foco/conexión y cada tres segundos mientras está visible. Bloquea acciones hasta confirmar la cuenta y durante escrituras; tras un fallo vuelve a leer antes de permitir reintentos. Los precios sintéticos no ejecutan operaciones de cuentas conectadas. Sin configuración Supabase se conserva la práctica local. `simulatorOwnerId` identifica la caché local y evita importarla al cambiar de cuenta. Si el servidor aún no tiene documento, se importa una operación/historial anterior desde su dispositivo original; una visita vacía desde otro dispositivo no bloquea esa importación. Si ya hay documento remoto, prevalece. La comparación entre documento local y remoto ordena las claves: `jsonb` las devuelve en su propio orden, así que comparar cadenas JSON directamente daba siempre «ha cambiado» y cada sondeo reescribía el store y guardaba el perfil. Sin claves de Supabase el panel avisa de que la operación solo existe en ese dispositivo, para no confundir una build sin backend con una pérdida de la posición.

Despliegue: ejecutar `0016_simulator_sync.sql` en el proyecto Supabase correspondiente antes de activar la operativa de la nueva versión. El fallback del perfil permite seguir sincronizando aprendizaje si falta la columna, pero el simulador conectado permanece bloqueado si faltan las RPC. Los dispositivos deben usar la misma cuenta, backend y versión actualizada. Los binarios nativos anteriores conservan su operativa local hasta actualizarse.
